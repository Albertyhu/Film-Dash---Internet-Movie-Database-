...
const PopulateMovie = (MovieArray) => {
    async.parallel(
        ...
        (err, results) => {
            if (err) {
                console.log("Error in retrieving movie information from database: ", err)
                return;
            }
            MovieArray.movies.forEach(item => {
                ...
                if (!Array.isArray(item.director)) {
                    directorObj = results.GetDirectors.find(val => val.name == item.director)._id
                }
                else {
                    directorObj = []; 
                    item.director.forEach(coDirector => {
                        directorObj.push(results.GetDirectors.find(val => val.name == coDirector)._id)
                    })
                }
                ...
                try {
                    item.actors.forEach(actor => {
                        ActorIDArray.push(results.GetActors.find(val => val.name == actor)._id)
                    })
                ...
                try {
                    item.genres.forEach(genre => {
                        GenreIDArray.push(results.GetGenres.find(val => val.name == genre)._id)
                    })
                } catch (e) {
